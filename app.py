...
# Forensics modules (Step 2-5 of Cyber Forensics report)
from forensics.hashing import calculate_sha256, compare_hashes, save_hash_record, verify_file_integrity
from forensics.frame_analysis import check_frame_continuity, extract_video_metadata
from forensics.metadata_inspector import extract_metadata, detect_metadata_anomalies
from forensics.tampering_simulator import delete_frames, cut_video, reencode_video
from forensics.report_generator import generate_forensic_report

# In-memory hash store for reference hashes (keyed by case_id)
_reference_hashes: Dict[str, str] = {}
# In-memory forensic reports store
_forensic_reports: Dict[str, Dict] = {}
...
# Implements the Cyber Forensics report methodology:
#   Step 2 — Evidence Preservation (SHA-256 Hashing)
...
async def generate_hash_endpoint(file: UploadFile = File(...)):
    """
    [FORENSICS Step 2] Compute SHA-256 hash of uploaded video.

    This implements Evidence Preservation — generating a cryptographic
    hash value to ensure video integrity, as described in the report:
    ...
    Returns the hash and saves a hash record for chain of custody.
    """
    settings = get_settings()
    content = await file.read()
    ...
    try:
        ...
        case_id = f"CF-{uuid.uuid4().hex[:8].upper()}"
        record = save_hash_record(
            file_path=temp_path,
            hash_value=hash_value,
            output_dir=settings.output_dir,
            case_id=case_id,
        )
        # Store reference hash in memory for later verification
        _reference_hashes[case_id] = hash_value
        REQUEST_COUNT.labels(endpoint="/generate-hash", status="success").inc()
        return {
            "case_id": case_id,
            "filename": file.filename,
            "sha256": hash_value,
            "file_size_bytes": len(content),
            "record_saved": record.get("_saved_to"),
            "message": "Hash generated and saved. Use case_id to verify integrity later.",
        }
    finally:
        if os.path.exists(temp_path):
            os.unlink(temp_path)
...
async def verify_integrity_endpoint(
    ...
    """
    [FORENSICS Step 2] Verify video integrity against a reference hash.

    Compare uploaded video's current hash against the original (stored hash).
    If hashes differ, the video has been tampered with.

    Parameters:
        reference_hash: Directly provide the known-good SHA-256 hash, OR
        case_id:        Lookup hash from a previously generated case.
    """
    settings = get_settings()
    content = await file.read()

    # Resolve reference hash
    ref_hash = reference_hash
    if not ref_hash and case_id:
        ref_hash = _reference_hashes.get(case_id)
    if not ref_hash:
        raise HTTPException(
            status_code=400,
            detail="Provide either reference_hash or a valid case_id from /generate-hash.",
        )
    ...
    try:
        result = verify_file_integrity(temp_path, ref_hash)
        result["filename"] = file.filename
        REQUEST_COUNT.labels(endpoint="/verify-integrity", status="success").inc()
        return result
    finally:
        if os.path.exists(temp_path):
            os.unlink(temp_path)
...
@app.post("/simulate-tampering")
async def simulate_tampering_endpoint(
    file: UploadFile = File(...),
    technique: str = "frame_deletion",
    start_param: float = 0.0,
    end_param: float = 2.0,
    codec: str = "XVID",
):
    """
    [FORENSICS Step 3] Simulate tampering on an uploaded video.

    Implements the three tampering techniques from the report:
      1. frame_deletion — Remove frames to hide events
      2. video_cutting  — Remove a timeline segment
      3. re_encoding    — Re-save with different compression

    Parameters:
        technique:   'frame_deletion', 'video_cutting', or 're_encoding'
        start_param: For deletion/cutting: start frame or start second
        end_param:   For deletion/cutting: end frame or end second
        codec:       For re_encoding: 'XVID', 'MJPG', 'mp4v'
    """
    settings = get_settings()
    content = await file.read()

    valid_techniques = ["frame_deletion", "video_cutting", "re_encoding"]
    ...
    output_path = os.path.join(settings.temp_dir, f"tampered_{uuid.uuid4().hex[:8]}{suffix}")

    try:
        if technique == "frame_deletion":
            sim_result = delete_frames(
                temp_path, output_path,
                start_frame=int(start_param),
                end_frame=int(end_param),
            )
        elif technique == "video_cutting":
            sim_result = cut_video(
                temp_path, output_path,
                cut_start_sec=float(start_param),
                cut_end_sec=float(end_param),
            )
        else:  # re_encoding
            sim_result = reencode_video(temp_path, output_path, codec=codec)

        # Compute hashes of both original and tampered
        orig_hash = calculate_sha256(temp_path)
        if os.path.exists(sim_result["output_path"]):
            tampered_hash = calculate_sha256(sim_result["output_path"])
            hash_comparison = compare_hashes(orig_hash, tampered_hash)
        else:
            hash_comparison = {"match": None, "verdict": "OUTPUT FILE MISSING"}

        REQUEST_COUNT.labels(endpoint="/simulate-tampering", status="success").inc()
        return {
            "technique": technique,
            "simulation_result": sim_result,
            "original_hash": orig_hash,
            "tampered_hash": hash_comparison.get("evidence_hash", "N/A"),
            "hash_comparison": hash_comparison,
            "forensic_note": (
                "Hash differs — tampering is detectable by hash verification."
                if not hash_comparison.get("match") else
                "WARNING: Hash unchanged — tampering may not be detectable by hash alone."
            ),
        }
    finally:
        if os.path.exists(temp_path):
            os.unlink(temp_path)
        # Clean tampered file after response (don't persist)
...
async def forensic_analyze_endpoint(
    request: Request,
    file: UploadFile = File(...),
    case_id: Optional[str] = None,
    ...
    """
    [FORENSICS Full Pipeline] Run the complete 5-step forensic analysis.

    Implements the full methodology from the Cyber Forensics report:
      Step 1: Video Acquisition
      Step 2: Evidence Preservation (SHA-256 hash)
      Step 3: N/A (simulation is separate endpoint)
      Step 4: Forensic Analysis (frame + metadata + AI)
      Step 5: Result Generation (forensic report)

    Parameters:
        case_id:        Optional case identifier
        analyst:        Optional analyst name
        reference_hash: Optional known-good hash to compare against
        run_ai:         Whether to run AI anomaly detection (default: true)
    """
    start_time = time.perf_counter()
    settings = get_settings()
    content = await file.read()

    case_id = case_id or f"CF-{uuid.uuid4().hex[:8].upper()}"
    ...
    try:
        ...
        _reference_hashes[case_id] = current_hash

        if reference_hash:
            hash_result = compare_hashes(reference_hash, current_hash)
        else:
            hash_result = {
                "match": None,
                "verdict": "HASH RECORDED — no reference provided for comparison",
                "reference_hash": "N/A",
                "evidence_hash": current_hash,
            }

        # Step 4a: Frame analysis
        loop = asyncio.get_event_loop()
        ...
        # Step 4b: Metadata inspection
        meta = extract_metadata(temp_path)
        meta_anomalies = detect_metadata_anomalies(meta)

        # Step 4c: AI anomaly detection
        if run_ai and _detector is not None:
            ...
            frame_count, anomaly_count, scores, flags, proc_time = ai_raw
            ai_result = {
                "frame_count": frame_count,
                "anomaly_count": anomaly_count,
                "anomaly_rate": anomaly_count / frame_count if frame_count > 0 else 0,
        ...
        # Store report
        _forensic_reports[case_id] = report
        report["_filename"] = file.filename
        report["_sha256"] = current_hash
        report["_total_time_sec"] = round(time.perf_counter() - start_time, 3)
        report["_metadata_analysis"] = meta_anomalies

        REQUEST_COUNT.labels(endpoint="/forensic-analyze", status="success").inc()
        return report

    except Exception as e:
        logger.exception(f"Forensic analysis failed: {e}")
...
@app.get("/forensic-report/{case_id}")
async def get_forensic_report(case_id: str):
    """
    [FORENSICS Step 5] Retrieve a previously generated forensic report by case ID.
    """
    if case_id not in _forensic_reports:
        raise HTTPException(
            status_code=404,
            detail=f"No forensic report found for case {case_id}. Run /forensic-analyze first."
        )
    return _forensic_reports[case_id]
...
async def root():
    ...
    For the interactive dashboard, visit the Streamlit app.
    """
